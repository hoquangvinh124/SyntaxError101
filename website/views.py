...
@views.route('/changeInformation', methods=['POST'])
...
# Nếu có upload file, cập nhật đường dẫn tương ứng
    ...
    return redirect(url_for('views.query_candidate'))
...
def query_candidate():
    ...
    search_status = request.args.get('experiencefilter')
    search_date_created = request.args.get('recencyfilter')

    # Gọi hàm search_candidates
    candidates = search_candidates('website/data/candidates/candidates.json', search_name, search_status)
    
    return render_template('tables_jinja.html', candidates=candidates)

@views.route('/createcandidate', methods=['POST'])
def create_candidate():
    """
    Nhận dữ liệu form (bao gồm file upload) từ client:
    - Các trường text: name, dob, gender, email, phone, identityCard, 
        workExperience, education, position, status, description
...
@views.route('edit/<candidate_id>', methods=['GET'])
